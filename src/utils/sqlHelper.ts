...
export function validateSQL(query: string): SQLValidationResult {
  ...
  if (singleQuotes % 2 !== 0) {
    return {
      ...
      suggestion:
        "Vérifiez que toutes les chaînes de caractères sont correctement quotées",
    };
  }
  ...
  if (trimmedQuery.startsWith("SELECT")) {
    ...
    if (!trimmedQuery.includes("WHERE") && !trimmedQuery.includes("LIMIT")) {
      warnings.push(
        "Considérez ajouter une clause WHERE ou LIMIT pour limiter les résultats",
      );
    }

    if (trimmedQuery.includes("ORDER BY") && !trimmedQuery.includes("LIMIT")) {
      warnings.push(
        "ORDER BY sans LIMIT peut être coûteux sur de grands jeux de données",
      );
    }
  ...
  const foundDangerous = DANGEROUS_KEYWORDS.filter(
    (keyword) =>
      (trimmedQuery.includes(keyword) && !trimmedQuery.startsWith("SELECT")) ||
      keyword === "UNION",
  );
...
export function analyzeQuery(query: string): {
  ...
  complexityScore += (
    upperQuery.match(/\b(COUNT|SUM|AVG|MIN|MAX|GROUP_CONCAT)\b/g) || []
  ).length;
...
export function explainQuery(query: string): {
  ...
  if (
    ...
    !upperQuery.match(/WHERE.*\s+(AND|OR)\s+.*=/)
  ) {
    optimizations.push(
      "Ajoutez des index sur les colonnes utilisées dans WHERE",
    );
  }

  if (upperQuery.includes("ORDER BY") && !upperQuery.includes("LIMIT")) {
    optimizations.push(
      "Ajoutez LIMIT après ORDER BY si vous n'avez pas besoin de tous les résultats",
    );
  }

  if (upperQuery.includes("JOIN") && !upperQuery.includes("ON")) {
    optimizations.push(
      "Vérifiez que les jointures utilisent des index appropriés",
    );
  }

  if (upperQuery.includes("SUBQUERY") || query.match(/\(.*SELECT.*\)/)) {
    optimizations.push(
      "Considérez convertir les sous-requêtes en JOIN si possible",
    );
  }
