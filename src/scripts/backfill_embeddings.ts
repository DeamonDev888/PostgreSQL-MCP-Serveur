...
async function backfillEmbeddings() {
  ...
  try {
    ...
    Logger.info(
      `🎯 Found ${totalToProcess} recent articles (last ${daysToLookBack} days) needing embeddings.`,
    );
    ...
    const probeVector = await embeddingService.generateEmbedding("Probe", {
      model: "qwen/qwen3-embedding-8b",
    });
    const apiDims = probeVector.length;
    Logger.info(
      `📏 API Model 'qwen/qwen3-embedding-8b' returns ${apiDims} dimensions.`,
    );
    ...
    if (apiDims !== 1536) {
      // Assuming 1536 is the old default
      Logger.warn(
        `⚠️ Dimension Mismatch! API: ${apiDims} vs DB Default (1536). Initiating Migration...`,
      );
      try {
        await pool.query(
          `ALTER TABLE enhanced_news ALTER COLUMN embedding TYPE vector(${apiDims})`,
        );
        Logger.info(
          `✅ Schema Updated: embedding column is now vector(${apiDims})`,
        );
      } catch (e: any) {
        Logger.error(
          `❌ Migration Failed: ${e.message}. Attempting to proceed anyway...`,
        );
      }
    }
